...
* **Story 4.1:** Create the `Model` Abstract Base Class (ABC) enforcing `train`, `predict`, and `save`.
* **Story 4.2:** Implement Reference **Stateful Model** (Elo) handling season-to-season state persistence.
* **Story 4.3:** Implement Reference **Stateless Model** (XGBoost wrapper).
