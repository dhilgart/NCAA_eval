...
- `specs/research/tournament-simulation-confidence.md` — NEW: Research findings document (primary deliverable); MODIFIED by code review round 1 (7 fixes) and round 2 (7 more fixes)
- `_bmad-output/planning-artifacts/template-requirements.md` — MODIFIED: Added 3 new learnings from round-2 review (mathematical claim verification, NFR1 in pseudocode, protocol type contracts)
...
- 2026-02-23: Code review round 1 (code-review workflow, YOLO mode). Found 2 HIGH + 5 MEDIUM + 3 LOW issues in the research document. Fixed all 7 HIGH/MEDIUM issues: (H1) replaced fictional `model.sample_from_posterior()` call with `provider_factory: Callable[[], ProbabilityProvider]` pattern; (H2) replaced non-vectorized per-pair probability loop with batched inference approach (NFR1-compliant); (M1) clarified `SimulationResult.expected_points` dict construction with example; (M2) documented `round_index` convention and accumulation safety invariant; (M3) corrected false claim that `serve_matchup_features_batch()` exists on `StatefulFeatureServer` (Story 6.5 must add it); (M4) added sections 7 and 8 to Quick-Navigation Table; (M5) added `season: int` parameter to `simulate_tournament_mc()` signature.
- 2026-02-23: Code review round 2 (code-review workflow, YOLO mode). Found 2 HIGH + 5 MEDIUM + 1 LOW issues. Fixed all 7 HIGH/MEDIUM issues: (H1) clarified `adv_probs[i, r]` semantics — "P(team i wins game in round r)", not ambiguous "advances past round r"; (H2) replaced misleading "vectorized" comment on a `for sim in range(n_simulations)` loop with explicit NFR1-compliant vectorization guidance for Story 6.5; (M1) corrected "384 advancement probabilities" claim to "64×6 matrix with 63 non-zero entries"; (M2) added `P(A)+P(B)=1` complementarity contract to `ProbabilityProvider` and `npt.NDArray[np.float64]` shape annotation to `batch_matchup_probabilities`; (M3) replaced arbitrary ±50–100 Elo noise heuristic with jackknife-based approach and K-factor-derived SE formula; (M4) added `score_distribution` field to `SimulationResult` and expanded MC scaffold with explicit vectorization TODO; (M5) added `MatchupContext` dataclass definition. Also applied L1: updated status header to "Code Review Applied (round 2)". Updated template-requirements.md with 3 new learnings.
