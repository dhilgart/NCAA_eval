...
- [ ] Task 5: Create LogisticRegression test fixture (AC: #9)
  - [ ] 5.1 Implement `LogisticRegressionModel(Model)` wrapping `sklearn.linear_model.LogisticRegression` in ~30 lines
  - [ ] 5.2 Implement `LogisticRegressionConfig(ModelConfig)` with minimal hyperparameters
  - [ ] 5.3 Register as `@register_model("logistic_regression")`
  - [ ] 5.4 Place in `src/ncaa_eval/model/logistic_regression.py`
  - [ ] 5.5 Implement `save`/`load` using `joblib.dump()`/`joblib.load()` + config JSON
