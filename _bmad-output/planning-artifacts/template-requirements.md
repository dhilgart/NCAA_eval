...
- ⚠️ **"Fold into any passing story" language also creates untracked action items** — The same problem as Cat-3 fix notes occurs when a PO decision says "fold into any passing story touching X file" (e.g., item 1.7: "add a docstring note to `serve_season_features()`... Fold into any passing story touching `feature_serving.py`"). Code reviewers must also check for this phrasing and ensure the action is added to the Post-MVP Backlog if no active story covers it. (Confirmed: Story 9.6 code review — 1.7 action item was untracked until review.)
