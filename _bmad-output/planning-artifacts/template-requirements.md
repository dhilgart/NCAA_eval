...
**Discovered in Story 9.13 code review (2026-03-11) — MagicMock `spec=` for Protocol mocks:**

When a `MagicMock` stands in for a typed Protocol or class, always pass `spec=TheClass` to prevent silent method-name typos from returning Mock objects instead of raising `AttributeError`:

```python
# Bad — typos silently return a Mock:
mock = MagicMock()
mock.serve_season_featurez(2020)  # → Mock() instead of AttributeError

# Good — typos raise AttributeError immediately:
mock = MagicMock(spec=StatefulFeatureServer)
mock.serve_season_featurez(2020)  # → AttributeError: Mock has no attribute 'serve_season_featurez'
```

Use `cast(TheClass, mock)` to satisfy mypy's return type checks when the mock helper's return type should be the Protocol type, not `MagicMock`.
...
- ⚠️ **"Prohibited project-wide" is a strong claim — verify against actual codebase before writing** — A code review fix that said `.itertuples()` is "prohibited project-wide" was itself incorrect: `espn.py`, `normalization.py`, and `model/base.py` all use `itertuples()` legitimately. The actual enforcement scope (only `evaluation/metrics.py`) must match the documented scope. Template pattern: Before writing any "project-wide prohibition" statement in docs, grep the codebase for actual usage to verify the claim. (Discovered: Story 9.15 Code Review Pass 2, 2026-03-12)
...
### MyST Admonition Nesting: Use 4-Backtick Outer Fence for Code Blocks Inside Admonitions (Discovered Story 7.9 Code Review)

In MyST Markdown, admonition directives opened with `` ```{tip} `` (triple backtick) **cannot** contain nested code blocks that also use triple backticks — the inner `` ```python `` fence prematurely closes the admonition, leaving the code content rendered as raw text outside the tip box.

```markdown
# BAD — inner ```python closes the outer ```{tip}
```{tip}
Some advice with a code example:
...
# GOOD — use 4-backtick outer fence so inner triple-backtick is valid
````{tip}
Some advice with a code example:
...
**Alternative:** Use `:::` fence syntax for admonitions (also valid in MyST), which allows any nested backtick fences inside.

**Template Action:** Any admonition (`{tip}`, `{note}`, `{warning}`) that contains a code block must use 4-backtick outer fence (`` ```` ``) or `:::` syntax.
...
A dev agent that runs `nox -s docs` as a quality gate must also commit the resulting `docs/api/*.rst` changes. Failing to do so leaves the repo in a dirty state and the generated files are not in the story File List.

**Key impact:** The `-f` flag overwrites existing files; `-e` creates per-module files. Every time new modules are added to the source tree (e.g., `cli/`, `evaluation/backtest.py`), `sphinx-apidoc` will create new `.rst` stubs. These must be staged and committed alongside any documentation story that runs `nox -s docs`.

**Template Action:** Add an explicit task in documentation stories: "Stage and commit `docs/api/*.rst` files generated by `nox -s docs` (sphinx-apidoc output)."
...
### Use `Model` ABC (Not `Any`) for `RunStore` Persistence Methods (Discovered Story 7.4 Code Review, 2026-02-24)

When adding persistence methods that accept or return model objects, always use the `Model` ABC type rather than `Any`:

```python
# ❌ Loses type safety — mypy can't catch wrong types being passed
def save_model(self, run_id: str, model: Any, ...) -> None: ...
def load_model(self, run_id: str) -> Any | None: ...

# ✅ Proper typing — mypy enforces Model contract (save/load classmethods)
from ncaa_eval.model.base import Model

def save_model(self, run_id: str, model: Model, ...) -> None: ...
def load_model(self, run_id: str) -> Model | None: ...
```

The deferred import inside the method body (`from ncaa_eval.model.registry import get_model`) avoids circular imports at module load time while the class-level `from ncaa_eval.model.base import Model` is safe since `base.py` does not import from `tracking.py`.

### Guard Feature Names / Importances Length Mismatch in Dashboard Loaders (Discovered Story 7.4 Code Review, 2026-02-24)

When zipping feature names with importance arrays, always check both that feature names are non-empty AND that lengths match before zipping:

```python
# ❌ Silent truncation / empty result when feature_names is []
feature_names = store.load_feature_names(run_id) or []
pairs = sorted(zip(feature_names, importances.tolist()), ...)

# ✅ Explicit guard — returns [] rather than silently producing wrong results
if importances is None or not feature_names or len(feature_names) != len(importances):
    return []
pairs = sorted(zip(feature_names, importances.tolist()), ...)
```

This matters when `save_model()` was called without `feature_names=` (legitimate legacy behavior).

### Keep `RunStore` Class Docstring Up-to-Date With Actual Directory Layout (Discovered Story 7.4 Code Review, 2026-02-24)

When new persistence methods add files to the run directory, always update the `RunStore` class docstring layout comment. Stale layout comments mislead future developers about what files to expect. Template pattern: the docstring layout should serve as the canonical specification of the run directory contract.

### Test CLI Persistence Beyond Metadata — Assert New Artifact Files Exist (Discovered Story 7.4 Code Review, 2026-02-24)

When the CLI acquires new persistence responsibilities (e.g., `fold_predictions.parquet`, `model/`), always extend the CLI integration tests to assert those new files exist. Dev Notes may call this out explicitly ("extend `test_cli_train.py` to assert `fold_predictions.parquet` and `model/` are created") — treat that as a mandatory task, not a suggestion.

### Use Pandas-Idiomatic `not df.empty` Instead of `len(df) > 0` (Discovered Story 7.4 Code Review, 2026-02-24)

```python
# ❌ Triggers pandas FutureWarning in newer versions; less readable
if len(filtered) > 0:
...
### sklearn Estimator Fit-State Guard: Use `hasattr` Not Just Attribute Truthiness (Discovered Story 9.3 Code Review, 2026-03-09)

When a model wraps a sklearn estimator and exposes a method that accesses a fit-time attribute (e.g., `coef_`, `feature_importances_`, `classes_`), guard against the unfitted case using `hasattr` — not just checking a boolean attribute like `_is_fitted` or a list like `feature_names_`.

**Why:** sklearn sets fit-time attributes _only after_ calling `.fit()`. If the guard only checks a proxy indicator (e.g., `if not self.feature_names_:`), a mismatch is possible — for example, if `feature_names_` is restored from disk before `_clf` is fully loaded, or if a test directly mutates `feature_names_`. Accessing `_clf.coef_` on an unfitted `LogisticRegression` raises `AttributeError`, which is not caught by the typical `(OSError, KeyError)` handler in callers.

```python
# ❌ Incomplete guard — only checks proxy, not actual estimator state
def get_feature_importances(self) -> list[tuple[str, float]] | None:
    if not self.feature_names_:
        return None
    coefs = np.abs(self._clf.coef_[0])  # AttributeError if _clf not fitted

# ✅ Correct: guard both proxy AND actual estimator attribute
def get_feature_importances(self) -> list[tuple[str, float]] | None:
...
**Applies to:** Any model method that accesses sklearn fit-time attributes (`coef_`, `feature_importances_`, `classes_`, `n_features_in_`, etc.). Compare against XGBoost which uses a dedicated `_is_fitted` flag set explicitly in `fit()` — that's equally robust as long as the flag is always set.

### Dashboard Sort Tests Must Use Out-of-Order Input (Discovered Story 9.3 Code Review, 2026-03-09)

When testing a data loading function that sorts its output (e.g., `load_feature_importances` sorts descending by `importance`), always provide deliberately out-of-order mock data. If mock data is already in the expected sorted order, the sort is never exercised and the test is vacuously satisfied.

```python
# ❌ Weak test: mock already sorted — sort bug would still pass
mock_model.get_feature_importances.return_value = [("team_100", 1600.0), ("team_200", 1400.0)]
assert result[0]["importance"] == 1600.0  # passes even if sort is broken

# ✅ Correct: mock in reverse order — sort must work for test to pass
mock_model.get_feature_importances.return_value = [("team_200", 1400.0), ("team_100", 1600.0)]
assert result[0]["feature"] == "team_100"  # only passes if sort is applied
```
...
### Vectorize N×N Matrix Construction With Broadcasting + np.vectorize (Discovered Story 9.7 Code Review, 2026-03-10)

**Anti-pattern:** Building an (n×n) probability matrix with a nested Python loop:
```python
for i in range(n):
    for j in range(i + 1, n):
        seed_diff = abs(seeds[i] - seeds[j])
        p = lookup(seed_diff)
        S[i, j] = p if seeds[i] < seeds[j] else 1.0 - p
```

**Rule:** Whenever an (n×n) matrix can be derived purely from element-wise functions of row and column indices (seeds, IDs, etc.), use NumPy broadcasting to vectorize the construction. The project style guide (Section 6.2) mandates no loops over matrices.

**Vectorized pattern:**
```python
seeds = np.array([seed_map.get(tid, 0) for tid in team_ids])
diff_matrix = np.abs(seeds[:, None] - seeds[None, :])        # (n, n) all-pairs diff
p_higher = np.vectorize(lookup_fn)(diff_matrix)               # (n, n) P(higher seed wins)
higher_mask = seeds[:, None] < seeds[None, :]                  # True where row is higher seed
lower_mask  = seeds[:, None] > seeds[None, :]
S = np.where(higher_mask, p_higher, np.where(lower_mask, 1.0 - p_higher, p_higher))
np.fill_diagonal(S, 0.0)
```

**Note:** `np.vectorize` is a convenience wrapper, not a speed optimization (it still loops internally). For large n, consider precomputing a lookup array indexed by diff. For n=64, `np.vectorize` is fast enough (< 1ms) and produces clean, readable code.

### Always Warn When Lookup Keys Fall Back to Defaults (Discovered Story 9.7 Code Review, 2026-03-10)

**Anti-pattern:** `seed_map.get(tid, 0)` silently falls back to `seed=0` if a team is missing from the map. When `seed_diff=0`, the prior returns 0.5 (coin flip), which is mathematically valid but data-masking — a caller error that surfaces as incorrect probabilities.

**Rule:** Whenever a map lookup falls back to a non-error default that can silently produce wrong results, add a `logger.warning()` listing the missing keys. The warning cost is zero for the happy path and invaluable for debugging malformed data.

```python
missing = [tid for tid in team_ids if tid not in seed_map]
if missing:
    logger.warning("Missing %d team(s) from seed_map — defaulting to seed=0: %s", len(missing), missing[:10])
```

**Applies to:** Any function that builds matrices from ID→attribute lookups (seed_map, team_name_map, etc.).

### Do Not Pass Ignored Parameters To Cached Functions That Affect The Cache Key (Discovered Story 9.7 Code Review, 2026-03-10)

See also the earlier entry: "**`@st.cache_data` key includes ALL parameters — normalize method-independent params before caching**" (Story 7.5).
