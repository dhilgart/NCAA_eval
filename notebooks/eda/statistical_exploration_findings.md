...
- **Strength of Schedule (SoS)**: mean regular-season opponent win rate, computed from 41 seasons (1985–2024)
- SoS ↔ Tournament Round Reached: **Pearson r = 0.2970** (signal = MEDIUM)
...
- **Approx. normal (no transform needed):** Score, FGM, FGA, DR, Ast, FGA3, PF — central-limit effect from summing many possession events per game.
- **Mildly right-skewed (sqrt + RobustScaler):** Blk, Stl, TO, FTM, OR, FGM3, FTA — moderate skew; sqrt transform brings them closer to normal. Era effects present for FGM3/FGA3 (higher volumes post-2010).
- **Bounded [0,1] (logit + StandardScaler):** FGPct, 3PPct, FTPct, TO_rate — Beta-like distributions; standard Normal/log transforms are inappropriate.
- **Men vs. Women:** Shape labels consistent across genders. Women's data shows higher FTPct means and lower FGM3/FGA3 (smaller historical 3-point volume).
- **Regular Season vs. Tournament:** Same shape, tighter spread in tournament (more evenly-matched teams). Transform recommendations unchanged across dataset types.
